...
from cochlear.rdt_parser.cr230.tagcode import base_type
...
class Code_subprogram(AbstractTagcode):
    ...
    def _get_from_die(self):
        if self.name is not None and self.name.startswith("$Sub$$"):
            self.name = self.name.replace("$Sub$$", "")
        ...
        try:
            if(self.project.ram_project is True and (self.project.symtab[self.name] < (self.project.hex_file.minaddr() + 3))):
                Code_subprogram.first_function = self.name
        except KeyError:
            pass
