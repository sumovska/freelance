...
class File(object):
    ...
    def __init__(self, project, name):
        ...
        self.file_generator = ProjectFile(self.name)
    ...
    def add_symbol(self, name, symbol, size=None):
        if isinstance(symbol.ctype, TypeFunction):
            ...
            if name.endswith("?_"):
                ...
                name = name[:-1]
            self._add_function(name, symbol, size, reentrant)
        ...
        for typedef in set(self.typedefs):
            ...
            for field in typeobject.fields:
                if isinstance(field.type, (TypeStruct, TypeUnion)):
                    ...
                    while isinstance(current_type.basictype, TypeArray):
                        current_type = current_type.basictype
                    if isinstance(current_type.basictype, (TypeStruct, TypeUnion)):
                        self._add_typedef(current_type.basictype)
                        typedependencies.add(current_type.basictype.name
                                             .replace("struct ", "", 1)
                                             .replace("union ", "", 1))
            if typedependencies:
                ...
                for field in typeobject.fields:
                    if isinstance(field.type, (TypeStruct, TypeUnion)):
                        ...
                        if isinstance(basetype, IntegerType):
                            ...
                            field_type = (field.type.basictype.name.replace("struct ", "", 1).replace("union ", "", 1))
                            if field_type in self.type_namedict:
                                field_type = self.type_namedict[field_type]
    ...
    def _add_function(self, name, symbol, size, reentrant):
        params_list = []
        memory_params = []
        ...
        if symbol.ctype.params is not None:
            ...
            if (symbol.module, symbol.name) in self.project.omf.arguments:
                at_namelist = self.project.omf.arguments[(symbol.module,
                                                          symbol.name)]
                for j, at_name in enumerate(at_namelist):
                    param_name, _, param_address = at_name.partition("@")
                    if param_address:
                        try:
                            i = namelist.index(param_name)
                        except ValueError:
                            i = j
            ...
            for param, param_name in zip(params, namelist[:len(params)]):
                ...
                type_size_counter += param.type.size
                param_name, _, param_address = param_name.partition("@")
                if type_size_counter > 11:
                    memory_params.append(eval(param_address) if param_address else None)
        ...
        near = symbol.ctype.rec.mci == 0x05
        rom = (address & 0xFF0000) == 0xFF0000
        if near:
    ...
    def _add_integer_variable(self, name, symbol):
        ...
        variable = Variable(name,
                            var_typename,
                            symbol.address,
                            self.project.is_const(symbol.address))
