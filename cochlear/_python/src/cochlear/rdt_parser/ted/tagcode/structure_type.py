...
class Code_structure_type(AbstractTagcode):
    ...
    def _get_from_die(self):
        if self.name is None:
            self.name = "UnnamedStructure_" + str(Code_structure_type.name_counter)
            Code_structure_type.name_counter += 1
        self.fields = list()
        ...
        for die in self.die.iter_children():
            if die.tag == 'DW_TAG_member':
                ...
                field.parent = self
                if field.typedef.bytesize is None:
                    field.typedef.refresh()
        # calculate if structure is packed or not
        fields_size = sum(f.typedef.bytesize for f in self.fields)
        if fields_size == self.bytesize:
            self.packed = True
    ...
    def get_python_implementation(self, tab):
        text = "\n\n"
        text += tab * "    " + "class " + self.name + "(Structure):\n"
