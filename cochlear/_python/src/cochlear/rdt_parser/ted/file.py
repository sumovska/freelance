...
from cochlear.rdt_parser.ted.tagcode.base_type import Code_base_type
from cochlear.rdt_parser.ted.tagcode.pointer_type import Code_pointer_type
from cochlear.rdt_parser.ted.tagcode.const_type import Code_const_type
from cochlear.rdt_parser.ted.tagcode.typedef import Code_typedef
from cochlear.rdt_parser.ted.tagcode.enumeration_type import Code_enumeration_type
from cochlear.rdt_parser.ted.tagcode.enumerator import Code_enumerator
from cochlear.rdt_parser.ted.tagcode.formal_parameter import Code_formal_parameter
from cochlear.rdt_parser.ted.tagcode.subroutine_type import Code_subroutine_type
from cochlear.rdt_parser.ted.tagcode.structure_type import Code_structure_type
from cochlear.rdt_parser.ted.tagcode.variable import Code_variable
from cochlear.rdt_parser.ted.tagcode.subprogram import Code_subprogram
from cochlear.rdt_parser.ted.tagcode.union_type import Code_union_type
from cochlear.rdt_parser.ted.tagcode.unspecified_type import Code_unspecified_type
from cochlear.rdt_parser.ted.tagcode.volatile_type import Code_volatile_type
from cochlear.rdt_parser.ted.tagcode.member import Code_member
from cochlear.rdt_parser.ted.tagcode.array_type import Code_array_type
...
class File(object):
    ...
    def parse_cus(self):
        ...
        for cu in self.cus:
            ...
            if "DW_AT_macro_info" in cu.get_top_DIE().attributes:
                ...
                #Filter off unnecessary defines
                for k in macroinfo.defines.keys():
                    if k.startswith(self.MACRO_FILTER):
                        macroinfo.defines.pop(k)
    ...
    def parse_single_die(self, die, project_link=True):
        ...
        if die.offset not in self.project.offset_dict or not project_link:
            ...
            #  if there is no type at DIE DW_TAG_pointer_type object, then it will raise error
            ...
            if isinstance(i, Code_structure_type):
                ...
                for field in i.fields:
                    try:
                        ...
                        pass
                typedef = Structure(i.name, values, 1 if i.packed else None)
            elif isinstance(i, Code_union_type):
                values = OrderedDict()
                for field in i.fields:
                    values[field.name] = str(field.typedef.get_type().replace("<C>", ""))
