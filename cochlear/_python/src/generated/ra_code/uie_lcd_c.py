...
class dimming__structure(Structure):
    briVal = s16
    state = UIE_P_DimmingState_t
    dimStep = u8
    _pack_ = 1
    _fields_ = [
                ('briVal', s16),
...
class const():
    ...
    ### Enum values ###
    ###################
    UIE_LCD_REQ_DIMMING_START = 1
    UIE_LCD_REQ_DIMMING_CANCEL = 2
    UIE_P_TIMER_NOTIFICATION = 0
    UIE_P_TIMER_ANIMATION = 1
    UIE_P_TIMER_SOUND = 2
    UIE_P_TIMER_USER_INACTIVITY = 3
...
class Code(AbstractCode):
    ...
    os_sem_data = os_sem_data
    dimming__structure = dimming__structure
    ...
    def UIE_P_LcdInit(self, init_type):
        '''
        Arguments:
        -init_type - INT_InitType_t
        Return type:
        -Status
        Declaration line: 123
        '''
        pass

    def UIE_P_LcdCancelDimming(self, ):
        '''
        Arguments:
        Return type:
        -None
    ...
    def UIE_P_LcdContrastSet(self, lcdConLvl):
        '''
        Arguments:
        -lcdConLvl - u8
        Return type:
    ...
    def UIE_P_LcdTurnOffDim(self, ):
        '''
        Arguments:
        Return type:
        -None
        Declaration line: 222
        '''
        pass

    def UieLcdTask(self, p_arg):
        '''
        Arguments:
    ...
    def UIE_P_LcdStartDimming(self, step):
        '''
        Arguments:
        -step - u8
        Return type:
    ...
    def UIE_P_LcdBrightnessSet(self, lcdBrightness):
        '''
        Arguments:
        -lcdBrightness - u8
        Return type:
    ...
    def LcdFade(self, fadeIn, fadeStep):
        '''
        Arguments:
        -fadeIn - bool
        -fadeStep - u8
    ...
    def UIE_P_GetLcdDimmingState(self, ):
        '''
        Arguments:
        Return type:
        -UIE_P_DimmingState_t
    ...
    uieLcdReqMailBox = PointerType('OS_EVENT')
    OSTmrFree = INT16U
    ...
    def __init__(self, device):
        ...
        self.uieLcdReqMailBox = StaticVariable(device, PointerType('OS_EVENT'), 0x80008408L, False)
        ...
        self.uieLcdTaskStack = StaticVariable(device, OS_STK * 128, 0x8000e004L, False)
        ...
        self.dimming = StaticVariable(device, self.dimming__structure, 0x80008404L, False)
        ...
        ### Functions data ###
        ######################
        self.UIE_P_LcdInit = StaticFunction(device, 0x39b1e, thumb=1, name='UIE_P_LcdInit', return_type=Status, size=126, line=123, arg_list=[('init_type',INT_InitType_t)])
        self.UIE_P_LcdCancelDimming = StaticFunction(device, 0x39bae, thumb=1, name='UIE_P_LcdCancelDimming', return_type=None, size=14, line=204, arg_list=[])
        self.UIE_P_LcdContrastSet = StaticFunction(device, 0x399e0, thumb=1, name='UIE_P_LcdContrastSet', return_type=None, size=14, line=111, arg_list=[('lcdConLvl',u8)])
        self.UIE_P_LcdTurnOffDim = StaticFunction(device, 0x39bc2, thumb=1, name='UIE_P_LcdTurnOffDim', return_type=None, size=120, line=222, arg_list=[])
        self.UieLcdTask = StaticFunction(device, 0x39a3e, thumb=1, name='UieLcdTask', return_type=None, size=224, line=263, arg_list=[('p_arg',PointerType('void'))])
        self.UIE_P_LcdStartDimming = StaticFunction(device, 0x39b9c, thumb=1, name='UIE_P_LcdStartDimming', return_type=None, size=18, line=194, arg_list=[('step',u8)])
        self.UIE_P_LcdBrightnessSet = StaticFunction(device, 0x399d4, thumb=1, name='UIE_P_LcdBrightnessSet', return_type=None, size=12, line=101, arg_list=[('lcdBrightness',u8)])
        self.LcdFade = StaticFunction(device, 0x399ee, thumb=1, name='LcdFade', return_type=None, size=80, line=367, arg_list=[('fadeIn',bool),('fadeStep',u8)])
        self.UIE_P_GetLcdDimmingState = StaticFunction(device, 0x39bbc, thumb=1, name='UIE_P_GetLcdDimmingState', return_type=UIE_P_DimmingState_t, size=6, line=213, arg_list=[])
