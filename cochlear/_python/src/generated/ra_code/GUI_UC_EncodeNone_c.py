...
class Code(AbstractCode):
    ...
    def __init__(self, device):
        ...
        ### Functions data ###
        ######################
        self.GUI_UC_SetEncodeNone = StaticFunction(device, 0x313d6, thumb=1, name='GUI_UC_SetEncodeNone', return_type=None, size=8, line=102, arg_list=[])
        self._Encode = StaticFunction(device, 0x313d0, thumb=1, name='_Encode', return_type=c_int_le, size=6, line=70, arg_list=[('s',PointerType('c_byte')),('Char',c_ushort_le)])
        self._CalcSizeOfChar = StaticFunction(device, 0x313cc, thumb=1, name='_CalcSizeOfChar', return_type=c_int_le, size=4, line=58, arg_list=[('Char',c_ushort_le)])
        self._GetCharCode = StaticFunction(device, 0x313c4, thumb=1, name='_GetCharCode', return_type=c_ushort_le, size=4, line=35, arg_list=[('s',PointerType("c_byte"))])
        self._GetCharSize = StaticFunction(device, 0x313c8, thumb=1, name='_GetCharSize', return_type=c_int_le, size=4, line=46, arg_list=[('s',PointerType('c_byte'))])
