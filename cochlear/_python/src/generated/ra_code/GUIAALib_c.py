...
class Code(AbstractCode):
    ...
    def GUI_AA_HiRes2Pixel(self, HiRes):
        '''
        Arguments:
        -HiRes - c_int_le
        Return type:
    ...
    def GUI_AA_Init_HiRes(self, x0, x1):
        '''
        Arguments:
        -x0 - c_int_le
        -x1 - c_int_le
    ...
    def GUI_AA_SetFactor(self, Factor):
        '''
        Arguments:
        -Factor - c_int_le
        Return type:
    ...
    abAABuffer = c_ubyte * 128
    GUI_ENC_APIList_EXT = tGUI_ENC_APIList
    pLCD_HLPrev = PointerType('tLCD_HL_APIList')
    _y = c_int_le
    ...
    DrawAPICopy = tLCD_HL_APIList
    _x0_InUse = c_int_le
    pabAABuffer = PointerType("c_ubyte")
    GUI_UC_None = GUI_UC_ENC_APILIST
    GUI_Font6x8 = GUI_FONT
    GUI_Pow10 = c_ulong_le * 10
    GUI_DecChar = c_byte
    ClipRect_HL = GUI_RECT
    ...
    def __init__(self, device):
        ...
        ### Variables ###
        #################
        self.abAABuffer = StaticVariable(device, c_ubyte * 128, 0x40005dec, False)
        self.GUI_ENC_APIList_EXT = StaticVariable(device, self.tGUI_ENC_APIList, 0x61c1c, True)
        self.pLCD_HLPrev = StaticVariable(device, PointerType('tLCD_HL_APIList'), 0x80008058L, False)
        self._y = StaticVariable(device, c_int_le, 0x8000804cL, False)
        ...
        self.DrawAPICopy = StaticVariable(device, self.tLCD_HL_APIList, 0x80008064L, False)
        self._x0_InUse = StaticVariable(device, c_int_le, 0x80008050L, False)
        self.pabAABuffer = StaticVariable(device, PointerType("c_ubyte"), 0x80008040L, False)
        ...
        self.ClipRect_HL = StaticVariable(device, self.GUI_RECT, 0x8000805cL, False)
        ...
        self._x1_InUse = StaticVariable(device, c_int_le, 0x80008054L, False)
        self.GUI_Context = StaticVariable(device, self.GUI_CONTEXT, 0x40005b98, False)

        ######################
        ### Functions data ###
        ######################
        self.GUI_AA_HiRes2Pixel = StaticFunction(device, 0x2e5cc, thumb=1, name='GUI_AA_HiRes2Pixel', return_type=c_short_le, size=24, line=260, arg_list=[('HiRes',c_int_le)])
        self.GUI_AA_Exit = StaticFunction(device, 0x2e5e4, thumb=1, name='GUI_AA_Exit', return_type=None, size=24, line=268, arg_list=[])
        self._FlushLine = StaticFunction(device, 0x2e5fc, thumb=1, name='_FlushLine', return_type=None, size=188, line=75, arg_list=[])
        self.GUI_AA_Init_HiRes = StaticFunction(device, 0x2e562, thumb=1, name='GUI_AA_Init_HiRes', return_type=c_int_le, size=38, line=217, arg_list=[('x0',c_int_le),('x1',c_int_le)])
        self._DrawHLine = StaticFunction(device, 0x2e3bc, thumb=1, name='_DrawHLine', return_type=None, size=296, line=115, arg_list=[('x0',c_int_le),('y',c_int_le),('x1',c_int_le)])
        self.GUI_AA_GetFactor = StaticFunction(device, 0x2e5b6, thumb=1, name='GUI_AA_GetFactor', return_type=c_int_le, size=6, line=236, arg_list=[])
        self.GUI_AA_Init = StaticFunction(device, 0x2e4e4, thumb=1, name='GUI_AA_Init', return_type=c_int_le, size=126, line=184, arg_list=[('x0',c_int_le),('x1',c_int_le)])
        self.GUI_AA_DisableHiRes = StaticFunction(device, 0x2e5bc, thumb=1, name='GUI_AA_DisableHiRes', return_type=None, size=8, line=244, arg_list=[])
        self.GUI_AA_SetFactor = StaticFunction(device, 0x2e588, thumb=1, name='GUI_AA_SetFactor', return_type=None, size=46, line=227, arg_list=[('Factor',c_int_le)])
        self.GUI_AA_EnableHiRes = StaticFunction(device, 0x2e5c4, thumb=1, name='GUI_AA_EnableHiRes', return_type=None, size=8, line=252, arg_list=[])
