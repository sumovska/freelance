...
class Code(AbstractCode):
    ...
    def UIE_IsAlarmPollingEnabled(self, ):
        '''
        Arguments:
        Return type:
        -bool
    ...
    def UIE_SetPwrMode(self, newPwrMode):
        '''
        Arguments:
    ...
    OSTime = INT32U
    uieKeyMailBox = PointerType("OS_EVENT")
    OSTCBFreeList = PointerType('OS_TCB')
    SQ_UieQueue = ITC_Queue_t * 8
    ...
    OSQTbl = OS_Q * 20
    OSCtxSwCtr = INT32U
    uieTaskStack = OS_STK * 1024
    OSTmrTime = INT32U
    OSTmrUsed = INT16U
    OSRdyGrp = INT8U
    SQ_UiePoolMem = ITC_MemPool_t * 12 * 8
    ...
    def __init__(self, device):
        ...
        self.uieKeyMailBox = StaticVariable(device, PointerType("OS_EVENT"), 0x800083b4L, False)
        self.OSTCBFreeList = StaticVariable(device, PointerType('OS_TCB'), 0x4000005c, False)
        self.SQ_UieQueue = StaticVariable(device, ITC_Queue_t * 8, 0x8000cd30L, False)
        ...
        self.uieMode = StaticVariable(device, self.bool, 0x800083b0L, False)
        ...
        self.uieTaskStack = StaticVariable(device, OS_STK * 1024, 0x8000cdb0L, False)
        ...
        self.SQ_UiePoolMem = StaticVariable(device, ITC_MemPool_t * 12 * 8, 0x8000cd50L, False)
        ...
        self.sm = StaticVariable(device, self.UIE_P_Sm_t, 0x8000ddb0L, False)
        self.OSTCBHighRdy = StaticVariable(device, PointerType('OS_TCB'), 0x40000060, False)

        ######################
        ### Functions data ###
        ######################
        self.UIE_Init = StaticFunction(device, 0x38a3c, thumb=1, name='UIE_Init', return_type=Status, size=352, line=105, arg_list=[('init_type',INT_InitType_t)])
        self.UIE_Fini = StaticFunction(device, 0x38b9c, thumb=1, name='UIE_Fini', return_type=Status, size=146, line=292, arg_list=[('fini_type',INT_FiniType_t)])
        self.UieTask = StaticFunction(device, 0x3898c, thumb=1, name='UieTask', return_type=None, size=176, line=444, arg_list=[('p_arg',PointerType("void"))])
        self.UIE_IsAlarmPollingEnabled = StaticFunction(device, 0x38c74, thumb=1, name='UIE_IsAlarmPollingEnabled', return_type=bool, size=34, line=430, arg_list=[])
        self.UIE_WakeUpByKeyPress = StaticFunction(device, 0x38c2e, thumb=1, name='UIE_WakeUpByKeyPress', return_type=None, size=12, line=370, arg_list=[])
        self.UIE_SetPwrMode = StaticFunction(device, 0x38c3a, thumb=1, name='UIE_SetPwrMode', return_type=None, size=58, line=386, arg_list=[('newPwrMode',UIE_PwrMode_t)])
