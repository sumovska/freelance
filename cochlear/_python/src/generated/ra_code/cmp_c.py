...
class Code(AbstractCode):
    ...
    def CMP_AllocateBufferLow(self, ppBufHndl):
        '''
        Arguments:
        -ppBufHndl - PointerType("PointerType('void')")
        Return type:
        -Status
    ...
    def CMP_WriteBuffer(self, pBufHndl, length, pData):
        '''
        Arguments:
        -pBufHndl - PointerType('void')
        -length - u16
    ...
    def CMP_ReleaseBufferLow(self, ppBufHndl):
        '''
        Arguments:
        -ppBufHndl - PointerType('PointerType("void")')
        Return type:
    ...
    def CMP_AllocateBuffer(self, ppBufHndl):
        '''
        Arguments:
        -ppBufHndl - PointerType("PointerType('void')")
        Return type:
    ...
    def CMP_ReleaseBuffer(self, ppBufHndl):
        '''
        Arguments:
        -ppBufHndl - PointerType('PointerType("void")')
        Return type:
    ...
    def CreateMemPool(self, pMemPool, poolSize, bufferSize):
        '''
        Arguments:
        -pMemPool - PointerType("u8")
        -poolSize - u8
        -bufferSize - u16
        Return type:
        -Status
    ...
    def CMP_QueryMemPool(self, pFree, pUsed):
        '''
        Arguments:
        -pFree - PointerType('u32')
        -pUsed - PointerType("u32")
        Return type:
        -Status
        Declaration line: 273
        '''
        pass
    ...
    OSTime = INT32U
    pMemPoolHandler = PointerType("OS_MEM")
    OSRdyTbl = INT8U * 8
    OSTCBHighRdy = PointerType('OS_TCB')
    OSEventTbl = OS_EVENT * 80
    commsMemoryPool = u8 * 1600 * 6
    ...
    def __init__(self, device):
        ...
        self.moduleInit = StaticVariable(device, self.bool, 0x80008a24L, False)
        ...
        self.pMemPoolHandler = StaticVariable(device, PointerType("OS_MEM"), 0x80008a28L, False)
        ...
        self.commsMemoryPool = StaticVariable(device, u8 * 1600 * 6, 0x400001b4, False)

        ######################
        ### Functions data ###
        ######################
        self.CMP_AllocateBufferLow = StaticFunction(device, 0x5b9de, thumb=1, name='CMP_AllocateBufferLow', return_type=Status, size=38, line=127, arg_list=[('ppBufHndl',PointerType("PointerType('void')"))])
        self.CMP_WriteBuffer = StaticFunction(device, 0x5ba1a, thumb=1, name='CMP_WriteBuffer', return_type=Status, size=32, line=185, arg_list=[('pBufHndl',PointerType('void')),('length',u16),('pData',PointerType("void"))])
        self.CMP_ReleaseBufferLow = StaticFunction(device, 0x5ba3a, thumb=1, name='CMP_ReleaseBufferLow', return_type=Status, size=36, line=211, arg_list=[('ppBufHndl',PointerType('PointerType("void")'))])
        self.CMP_Fini = StaticFunction(device, 0x5b9c8, thumb=1, name='CMP_Fini', return_type=Status, size=22, line=102, arg_list=[('finiType',INT_FiniType_t)])
        self.CMP_AllocateBuffer = StaticFunction(device, 0x5ba04, thumb=1, name='CMP_AllocateBuffer', return_type=Status, size=22, line=156, arg_list=[('ppBufHndl',PointerType("PointerType('void')"))])
        self.CMP_ReleaseBuffer = StaticFunction(device, 0x5ba5e, thumb=1, name='CMP_ReleaseBuffer', return_type=Status, size=16, line=244, arg_list=[('ppBufHndl',PointerType('PointerType("void")'))])
        self.FlushMemPool = StaticFunction(device, 0x5b9a0, thumb=1, name='FlushMemPool', return_type=Status, size=40, line=344, arg_list=[])
        self.CreateMemPool = StaticFunction(device, 0x5b950, thumb=1, name='CreateMemPool', return_type=Status, size=50, line=313, arg_list=[('pMemPool',PointerType("u8")),('poolSize',u8),('bufferSize',u16)])
        self.CMP_Init = StaticFunction(device, 0x5b982, thumb=1, name='CMP_Init', return_type=Status, size=30, line=80, arg_list=[('initType',INT_InitType_t)])
        self.CMP_QueryMemPool = StaticFunction(device, 0x5ba6e, thumb=1, name='CMP_QueryMemPool', return_type=Status, size=46, line=273, arg_list=[('pFree',PointerType('u32')),('pUsed',PointerType("u32"))])
