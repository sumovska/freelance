...
class Code(AbstractCode):
    ...
    def GUI_FillPolygon(self, pPoints, NumPoints, x0, y0):
        '''
        Arguments:
        -pPoints - PointerType('GUI_POINT')
        -NumPoints - c_int_le
        -x0 - c_int_le
        -y0 - c_int_le
        Return type:
    ...
    def GL_FillPolygon(self, paPoint, NumPoints, xOff, yOff):
        '''
        Arguments:
        -paPoint - PointerType("GUI_POINT")
        -NumPoints - c_int_le
        -xOff - c_int_le
        -yOff - c_int_le
        Return type:
    ...
    def __init__(self, device):
        ...
        self.GL_FP_Cnt = StaticVariable(device, c_int_le, 0x8000806cL, False)
        self.GUI_Context = StaticVariable(device, self.GUI_CONTEXT, 0x40005b98, False)
        self._aX = StaticVariable(device, c_short_le * 12, 0x40005e6c, False)

        ######################
        ### Functions data ###
        ######################
        self._DrawHLine = StaticFunction(device, 0x2e3bc, thumb=1, name='_DrawHLine', return_type=None, size=296, line=54, arg_list=[('x0',c_int_le),('y',c_int_le),('x1',c_int_le)])
        self.GUI_FillPolygon = StaticFunction(device, 0x2f47c, thumb=1, name='GUI_FillPolygon', return_type=None, size=2, line=263, arg_list=[('pPoints',PointerType('GUI_POINT')),('NumPoints',c_int_le),('x0',c_int_le),('y0',c_int_le)])
        self.GL_FillPolygon = StaticFunction(device, 0x2f23c, thumb=1, name='GL_FillPolygon', return_type=None, size=576, line=209, arg_list=[('paPoint',PointerType("GUI_POINT")),('NumPoints',c_int_le),('xOff',c_int_le),('yOff',c_int_le)])
