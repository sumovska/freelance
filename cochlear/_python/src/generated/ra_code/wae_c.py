...
class ELOG_PermanentEventType_tag(c_ubyte,Enumed):
    _ctype = c_ubyte
    ELOG_PERMANENT_EVENT_FIRST_PAIRING = 1
    ELOG_PERMANENT_EVENT_FW_UPGRADE = 2
...
ELOG_PermanentEventType_t = ELOG_PermanentEventType_tag
...
class LOG_PermanentFwUpgrade_tag(Structure):
    ffAppFwVer = u8 * 10
    _pack_ = 1
    _fields_ = [
                ('ffAppFwVer', u8 * 10),
...
class LOG_PermanentFirstPairing_tag(Structure):
    bteSerialNum = u8 * 10
    _pack_ = 1
    _fields_ = [
                ('bteSerialNum', u8 * 10),
...
LOG_PermanentFirstPairing_t = LOG_PermanentFirstPairing_tag
...
LOG_PermanentFwUpgrade_t = LOG_PermanentFwUpgrade_tag
...
class ELOG_P_CatPermanent_tag__union0(Union):
    firstPairing = LOG_PermanentFirstPairing_t
    fwUpgrade = LOG_PermanentFwUpgrade_t
    _fields_ = [
                ('firstPairing', LOG_PermanentFirstPairing_t),
                ('fwUpgrade', LOG_PermanentFwUpgrade_t),
               ]
...
class ELOG_P_CatPermanent_tag(Structure):
    timestamp = u8 * 5
    eventType = u8
    data = ELOG_P_CatPermanent_tag__union0
    _pack_ = 1
    _fields_ = [
                ('timestamp', u8 * 5),
                ('eventType', u8),
...
ELOG_P_CatPermanent_t = ELOG_P_CatPermanent_tag
...
class const():
    ...
    DSPI_1 = 2
    DSPI_FIFO_SIZE = 8
    LOG_PERMANENT_FIRST_PAIRING_SERIAL_NUM_LEN = 10
    LOG_PERMANENT_FW_VERSION_LEN = 10
    RTC_TIMESTAMP_LEN = 5
...
class Code(AbstractCode):
    ...
    ELOG_PermanentEventType_tag = ELOG_PermanentEventType_tag
    ...
    ELOG_PermanentEventType_t = ELOG_PermanentEventType_t
    ...
    LOG_PermanentFwUpgrade_tag = LOG_PermanentFwUpgrade_tag
    ...
    LOG_PermanentFirstPairing_tag = LOG_PermanentFirstPairing_tag
    ...
    LOG_PermanentFirstPairing_t = LOG_PermanentFirstPairing_t
    ...
    LOG_PermanentFwUpgrade_t = LOG_PermanentFwUpgrade_t
    OS_SEM_DATA = OS_SEM_DATA
    OS_TMR_WHEEL = OS_TMR_WHEEL
    OS_STK_DATA = OS_STK_DATA
    ...
    ELOG_P_CatPermanent_tag__union0 = ELOG_P_CatPermanent_tag__union0
    ...
    ELOG_P_CatPermanent_tag = ELOG_P_CatPermanent_tag
    ...
    ELOG_P_CatPermanent_t = ELOG_P_CatPermanent_t
    ...
    def IsSurgicalRA(self, hwUniqueDeviceNumber):
        '''
        Arguments:
    ...
    def WAE_InitSystem(self, init_type):
        '''
        Arguments:
        -init_type - INT_InitType_t
        Return type:
    ...
    def WAE_GetFwVersion(self, fwVer, legth):
        '''
        Arguments:
        -fwVer - PointerType('c_byte')
        -legth - PointerType("u8")
        Return type:
        -Status
    ...
    OSPrioHighRdy = INT8U
    waeItcPoolMem = ITC_MemPool_t * 28 * 8
    waeFFAppFwVer = u8 * 10
    ...
    OSLockNesting = INT8U
    waeTaskData = WAE_P_MainTaskData_t
    ...
    waeTaskStack = OS_STK * 512
    DAUDIO_SoundNumberOfRepetitions_t = u16 * 7
    waeFFAppWirelessFwVer = u8 * 3
    ...
    waeFFAppLogVer = u8 * 4
    waeNvmRW = WAE_NVM_RW_t
    OSRdyTbl = INT8U * 8
    WAE_P_FiniFun = INT_FiniFun_t * 20
    waeNvmRO = WAE_NVM_RO_t
    ...
    def __init__(self, device):
        ...
        self.waeItcQueue = StaticVariable(device, ITC_Queue_t * 8, 0x7fe0093c, False)
        ...
        self.waeInitAppType = StaticVariable(device, self.u32, 0xe00847f8L, False)
        ...
        self.waeItcPoolMem = StaticVariable(device, ITC_MemPool_t * 28 * 8, 0x7fe0095c, False)
        self.waeFFAppFwVer = StaticVariable(device, u8 * 10, 0x7034d, True)
        ...
        self.waeTaskData = StaticVariable(device, self.WAE_P_MainTaskData_t, 0x7fe00004, False)
        ...
        self.waeTaskStack = StaticVariable(device, OS_STK * 512, 0x7fe0013c, False)
        self.DAUDIO_SoundNumberOfRepetitions_t = StaticVariable(device, u16 * 7, 0x800089acL, False)
        self.waeFFAppWirelessFwVer = StaticVariable(device, u8 * 3, 0x70346, True)
        ...
        self.waeFFAppLogVer = StaticVariable(device, u8 * 4, 0x70349, True)
        self.waeNvmRW = StaticVariable(device, self.WAE_NVM_RW_t, 0x7fe00000, False)
        ...
        self.waeNvmRO = StaticVariable(device, self.WAE_NVM_RO_t, 0x7fe00a3c, False)
        ...
        ### Functions data ###
        ######################
        self.WAE_Fini = StaticFunction(device, 0x424a8, thumb=1, name='WAE_Fini', return_type=Status, size=48, line=524, arg_list=[('fini_type',INT_FiniType_t)])
        self.IsSurgicalRA = StaticFunction(device, 0x4220e, thumb=1, name='IsSurgicalRA', return_type=bool, size=36, line=573, arg_list=[('hwUniqueDeviceNumber',PointerType("u8"))])
        self.WAE_Init = StaticFunction(device, 0x42232, thumb=1, name='WAE_Init', return_type=Status, size=630, line=191, arg_list=[('init_type',INT_InitType_t)])
        self.WAE_InitSystem = StaticFunction(device, 0x421d0, thumb=1, name='WAE_InitSystem', return_type=Status, size=62, line=148, arg_list=[('init_type',INT_InitType_t)])
        self.WAE_GetFwVersion = StaticFunction(device, 0x424d8, thumb=1, name='WAE_GetFwVersion', return_type=Status, size=18, line=555, arg_list=[('fwVer',PointerType('c_byte')),('legth',PointerType("u8"))])
