...
class const():
    ...
    ###############
    EIGHT_BITS_SHIFT = 8
    IROM_BLOCK_LENGTH_ADDRESS = 163840
    WDOG_ON = 1
    WDOG_DURATION = 9000
...
class Code(AbstractCode):
    ...
    def RDT_CheckSecurity(self, securityField):
        '''
        Arguments:
        -securityField - u16
        Return type:
    ...
    def RDT_P_ParseDataPacketReq(self, pData, pReqHandler, pLength):
        '''
        Arguments:
    ...
    def ProcessRequestCallFunction(self, pReqHandler, reqLength):
        '''
        Arguments:
        -pReqHandler - PointerType('byte')
        -reqLength - u16
    ...
    OSTmrFree = INT16U
    rdtTaskStack = OS_STK * 512
    pFunctionCall = PointerType('Subroutine')
    OSTCBList = PointerType("OS_TCB")
    OSTmrSemSignal = PointerType('OS_EVENT')
    OSTime = INT32U
    rdtQueue = ITC_Queue_t * 2
    OSRdyTbl = INT8U * 8
    ...
    def __init__(self, device):
        ...
        self.rdtPoolMem = StaticVariable(device, ITC_MemPool_t * 20 * 2, 0x4000d234, False)
        ...
        self.rdtTaskStack = StaticVariable(device, OS_STK * 512, 0x4000d25c, False)
        self.pFunctionCall = StaticVariable(device, PointerType('Subroutine'), 0x80008b30L, False)
        ...
        self.rdtQueue = StaticVariable(device, ITC_Queue_t * 2, 0x80008b34L, False)
        ...
        ### Functions data ###
        ######################
        self.RDT_CheckSecurity = StaticFunction(device, 0x5cd70, thumb=1, name='RDT_CheckSecurity', return_type=RDT_Status_t, size=38, line=449, arg_list=[('securityField',u16)])
        self.RDT_Fini = StaticFunction(device, 0x5d00e, thumb=1, name='RDT_Fini', return_type=Status, size=34, line=193, arg_list=[('finiType',INT_FiniType_t)])
        self.RdtTask = StaticFunction(device, 0x5ceea, thumb=1, name='RdtTask', return_type=None, size=172, line=234, arg_list=[('pArg',PointerType('void'))])
        self.RDT_Init = StaticFunction(device, 0x5cf96, thumb=1, name='RDT_Init', return_type=Status, size=120, line=120, arg_list=[('initType',INT_InitType_t)])
        self.RDT_P_ParseDataPacketReq = StaticFunction(device, 0x5cec2, thumb=1, name='RDT_P_ParseDataPacketReq', return_type=RDT_Status_t, size=40, line=317, arg_list=[('pData',PointerType("VCH_DataPktReq_t")),('pReqHandler',PointerType('PointerType("byte")')),('pLength',PointerType("u16"))])
        self.ProcessRequestCallFunction = StaticFunction(device, 0x5cd96, thumb=1, name='ProcessRequestCallFunction', return_type=RDT_Status_t, size=300, line=346, arg_list=[('pReqHandler',PointerType('byte')),('reqLength',u16)])
