...
class const():
    ...
    ARM_MODE_ARM = 0
    ARM_MODE_THUMB = 32
    ARM_SVC_MODE_THUMB = 51
    ARM_SVC_MODE_ARM = 19
    OS_NTASKS_FP = 25
    OS_FP_STORAGE_SIZE = 128
    OS_VERSION = 284
    OS_FALSE = 0
    OS_TRUE = 1
    OS_PRIO_SELF = 255
...
class Code(AbstractCode):
    ...
    def OSTaskDelHook(self, ptcb):
        '''
        Arguments:
        -ptcb - PointerType("OS_TCB")
        Return type:
        -None
    ...
    def OSTimeTickHook(self, ):
        '''
        Arguments:
        Return type:
        -None
        Declaration line: 414
        '''
        pass

    def OSTaskStkInit(self, task, p_arg, ptos, opt):
        '''
        Arguments:
    ...
    def OSTaskCreateHook(self, ptcb):
        '''
        Arguments:
        -ptcb - PointerType('OS_TCB')
        Return type:
    ...
    def OSTaskIdleHook(self, ):
        '''
        Arguments:
        Return type:
        -None
        Declaration line: 246
        '''
        pass

    def OSTaskStatHook(self, ):
        '''
        Arguments:
        Return type:
        -None
    ...
    def OS_CPU_InitExceptVect(self, ):
        '''
        Arguments:
        Return type:
        -None
    ...
    def __init__(self, device):
        ...
        self.OSTmrCtr = StaticVariable(device, self.INT16U, 0x40000040, False)
        ...
        ### Functions data ###
        ######################
        self.OSTaskDelHook = StaticFunction(device, 0x284dc, thumb=1, name='OSTaskDelHook', return_type=None, size=2, line=219, arg_list=[('ptcb',PointerType("OS_TCB"))])
        self.OSTimeTickHook = StaticFunction(device, 0x28554, thumb=1, name='OSTimeTickHook', return_type=None, size=32, line=414, arg_list=[])
        self.OSTaskStkInit = StaticFunction(device, 0x284e8, thumb=1, name='OSTaskStkInit', return_type=PointerType("OS_STK"), size=104, line=304, arg_list=[('task',PointerType('Subroutine')),('p_arg',PointerType("void")),('ptos',PointerType('OS_STK')),('opt',INT16U)])
        self.OSTaskCreateHook = StaticFunction(device, 0x284da, thumb=1, name='OSTaskCreateHook', return_type=None, size=2, line=180, arg_list=[('ptcb',PointerType('OS_TCB'))])
        self.OSTaskSwHook = StaticFunction(device, 0x28550, thumb=1, name='OSTaskSwHook', return_type=None, size=2, line=353, arg_list=[])
        self.OSTaskIdleHook = StaticFunction(device, 0x284de, thumb=1, name='OSTaskIdleHook', return_type=None, size=8, line=246, arg_list=[])
        self.OSTaskStatHook = StaticFunction(device, 0x284e6, thumb=1, name='OSTaskStatHook', return_type=None, size=2, line=270, arg_list=[])
        self.OSTCBInitHook = StaticFunction(device, 0x28552, thumb=1, name='OSTCBInitHook', return_type=None, size=2, line=395, arg_list=[('ptcb',PointerType("OS_TCB"))])
        self.OS_CPU_InitExceptVect = StaticFunction(device, 0x28574, thumb=1, name='OS_CPU_InitExceptVect', return_type=None, size=48, line=494, arg_list=[])
        self.OSInitHookEnd = StaticFunction(device, 0x284d8, thumb=1, name='OSInitHookEnd', return_type=None, size=2, line=156, arg_list=[])
        self.OSInitHookBegin = StaticFunction(device, 0x284d0, thumb=1, name='OSInitHookBegin', return_type=None, size=8, line=135, arg_list=[])
