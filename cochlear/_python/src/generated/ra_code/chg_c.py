...
OS_TMR = os_tmr
OS_TCB = os_tcb
class CHG_Msg_t__structure(Structure):
    header = RPC_Msg_t
    params = u8 * 10
    _fields_ = [
                ('header', RPC_Msg_t),
...
CHG_Msg_t = CHG_Msg_t__structure
...
class Code(AbstractCode):
    ...
    OS_TMR = OS_TMR
    OS_TCB = OS_TCB
    CHG_Msg_t__structure = CHG_Msg_t__structure
    CHG_Msg_t = CHG_Msg_t
    ...
    def CHG_EnableFastCharging(self, status):
        '''
        Arguments:
        -status - bool
        Return type:
    ...
    def CHG_TemporarySuspendCharging(self, ):
        '''
        Arguments:
        Return type:
        -None
    ...
    OSTCBTbl = OS_TCB * 26
    OSTmrUsed = INT16U
    chgMemoryPool = PointerType('OS_MEM')
    ...
    chgProcQueue = PointerType("OS_EVENT")
    OSTime = INT32U
    OSRdyTbl = INT8U * 8
    ...
    def __init__(self, device):
        ...
        self.chgMemoryPool = StaticVariable(device, PointerType('OS_MEM'), 0x8000837cL, False)
        ...
        self.chgProcQueue = StaticVariable(device, PointerType("OS_EVENT"), 0x80008380L, False)
        ...
        ### Functions data ###
        ######################
        self.CHG_Update = StaticFunction(device, 0x34a3a, thumb=1, name='CHG_Update', return_type=Status, size=72, line=366, arg_list=[])
        self.CHG_Init = StaticFunction(device, 0x3481e, thumb=1, name='CHG_Init', return_type=Status, size=146, line=97, arg_list=[('initType',INT_InitType_t)])
        self.CHG_Finish = StaticFunction(device, 0x349aa, thumb=1, name='CHG_Finish', return_type=Status, size=72, line=296, arg_list=[])
        self.CHG_EnableFastCharging = StaticFunction(device, 0x348bc, thumb=1, name='CHG_EnableFastCharging', return_type=Status, size=90, line=186, arg_list=[('status',bool)])
        self.CHG_Terminate = StaticFunction(device, 0x349f2, thumb=1, name='CHG_Terminate', return_type=Status, size=72, line=331, arg_list=[])
        self.chgTask = StaticFunction(device, 0x347ec, thumb=1, name='chgTask', return_type=None, size=50, line=412, arg_list=[('param',PointerType("void"))])
        self.CHG_TemporarySuspendCharging = StaticFunction(device, 0x34a82, thumb=1, name='CHG_TemporarySuspendCharging', return_type=None, size=8, line=398, arg_list=[])
        self.CHG_Fini = StaticFunction(device, 0x348b0, thumb=1, name='CHG_Fini', return_type=Status, size=12, line=171, arg_list=[('finiType',INT_FiniType_t)])
        self.CHG_TurnOff = StaticFunction(device, 0x34962, thumb=1, name='CHG_TurnOff', return_type=Status, size=72, line=261, arg_list=[])
        self.CHG_TurnOn = StaticFunction(device, 0x34916, thumb=1, name='CHG_TurnOn', return_type=Status, size=76, line=226, arg_list=[])
