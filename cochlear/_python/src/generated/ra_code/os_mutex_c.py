...
class const():
    ...
    ### Defines ###
    ###############
    OS_MUTEX_KEEP_LOWER_8 = 255
    OS_MUTEX_KEEP_UPPER_8 = 65280
    OS_MUTEX_AVAILABLE = 255
    OS_VERSION = 284
    OS_FALSE = 0
    OS_TRUE = 1
    OS_PRIO_SELF = 255
...
class Code(AbstractCode):
    ...
    def OSMutexDel(self, pevent, opt, err):
        '''
        Arguments:
    ...
    def OSMutexPost(self, pevent):
        '''
        Arguments:
        -pevent - PointerType("OS_EVENT")
        Return type:
        -INT8U
        Declaration line: 546
        '''
        pass

    def OSMutexPend(self, pevent, timeout, err):
        '''
        Arguments:
        -pevent - PointerType('OS_EVENT')
        -timeout - INT16U
        ...
        Return type:
        -None
        Declaration line: 397
        '''
        pass

    def OSMutexAccept(self, pevent, err):
        '''
        Arguments:
        -pevent - PointerType('OS_EVENT')
        -err - PointerType("INT8U")
    ...
    def OSMutex_RdyAtPrio(self, ptcb, prio):
        '''
        Arguments:
    ...
    def OSMutexQuery(self, pevent, p_mutex_data):
        '''
        Arguments:
        -pevent - PointerType("OS_EVENT")
        -p_mutex_data - PointerType('OS_MUTEX_DATA')
        Return type:
        -INT8U
        Declaration line: 619
        '''
        pass

    def OSMutexCreate(self, prio, err):
        '''
        Arguments:
        -prio - INT8U
        -err - PointerType("INT8U")
    ...
    def __init__(self, device):
        ...
        ### Functions data ###
        ######################
        self.OSMutexDel = StaticFunction(device, 0x296fe, thumb=1, name='OSMutexDel', return_type=PointerType('OS_EVENT'), size=224, line=258, arg_list=[('pevent',PointerType('OS_EVENT')),('opt',INT8U),('err',PointerType("INT8U"))])
        self.OSMutexPost = StaticFunction(device, 0x29980, thumb=1, name='OSMutexPost', return_type=INT8U, size=200, line=546, arg_list=[('pevent',PointerType("OS_EVENT"))])
        self.OSMutexPend = StaticFunction(device, 0x297de, thumb=1, name='OSMutexPend', return_type=None, size=418, line=397, arg_list=[('pevent',PointerType('OS_EVENT')),('timeout',INT16U),('err',PointerType("INT8U"))])
        self.OSMutexAccept = StaticFunction(device, 0x295cc, thumb=1, name='OSMutexAccept', return_type=INT8U, size=110, line=84, arg_list=[('pevent',PointerType('OS_EVENT')),('err',PointerType("INT8U"))])
        self.OSMutex_RdyAtPrio = StaticFunction(device, 0x296a8, thumb=1, name='OSMutex_RdyAtPrio', return_type=None, size=86, line=683, arg_list=[('ptcb',PointerType('OS_TCB')),('prio',INT8U)])
        self.OSMutexQuery = StaticFunction(device, 0x29a48, thumb=1, name='OSMutexQuery', return_type=INT8U, size=96, line=619, arg_list=[('pevent',PointerType("OS_EVENT")),('p_mutex_data',PointerType('OS_MUTEX_DATA'))])
        self.OSMutexCreate = StaticFunction(device, 0x2963a, thumb=1, name='OSMutexCreate', return_type=PointerType('OS_EVENT'), size=110, line=166, arg_list=[('prio',INT8U),('err',PointerType("INT8U"))])
