...
class Code(AbstractCode):
    ...
    def ProcessRequest(self, pReqHandler, length):
        '''
        Arguments:
        -pReqHandler - PointerType('byte')
        -length - u16
    ...
    def ProcessResponse(self, eventId, pEventData, len):
        '''
        Arguments:
        -eventId - ITC_EventId_t
        -pEventData - PointerType('WacipMaxEventPayload_t')
        -len - u16
        Return type:
        -WACIP_Status_t
    ...
    def __init__(self, device):
        ...
        self.wacipQueue = StaticVariable(device, ITC_Queue_t * 5, 0x7fe00bec, False)
        ...
        self.wacipPoolMem = StaticVariable(device, ITC_MemPool_t * 88 * 5, 0x7fe00c00, False)
        ...
        self.wacipTaskStack = StaticVariable(device, OS_STK * 512, 0x7fe00db8, False)
        ...
        ### Functions data ###
        ######################
        self.ProcessRequest = StaticFunction(device, 0x52e38, thumb=1, name='ProcessRequest', return_type=WACIP_Status_t, size=418, line=507, arg_list=[('pReqHandler',PointerType('byte')),('length',u16)])
        self.WacipTask = StaticFunction(device, 0x531e0, thumb=1, name='WacipTask', return_type=None, size=480, line=236, arg_list=[('pArg',PointerType("void"))])
        self.WACIP_Init = StaticFunction(device, 0x533c0, thumb=1, name='WACIP_Init', return_type=Status, size=106, line=108, arg_list=[('initType',INT_InitType_t)])
        self.ProcessResponse = StaticFunction(device, 0x52fda, thumb=1, name='ProcessResponse', return_type=WACIP_Status_t, size=518, line=782, arg_list=[('eventId',ITC_EventId_t),('pEventData',PointerType('WacipMaxEventPayload_t')),('len',u16)])
        self.WACIP_Fini = StaticFunction(device, 0x5342a, thumb=1, name='WACIP_Fini', return_type=Status, size=40, line=194, arg_list=[('finiType',INT_FiniType_t)])
