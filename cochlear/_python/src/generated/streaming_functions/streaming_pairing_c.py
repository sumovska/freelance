...
class Proximity2Header_s(Structure):
    Pn = u8
    Dt = u8
    Di = u8
    Pp = u8
...
class StreamingBroadcastReq_t__structure(Structure):
    header = u8
    senderId = u8
    reserved = u8
    length = u8
...
Proximity2Header_t = Proximity2Header_s
OS_MEM_DATA = os_mem_data
StreamingBroadcastReq_t = StreamingBroadcastReq_t__structure
...
class const():
    ...
    ### Defines ###
    ###############
    STREAMING_PAIRED_BITMASK = 56
    STREAMING_PAIR_TIMEOUT = 4270
    STREAMING_PAIR_PFC_SWITCH_TIMEOUT = 50
    OS_VERSION = 284
    OS_FALSE = 0
    OS_TRUE = 1
    OS_PRIO_SELF = 255
...
class Code(AbstractCode):
    ...
    os_tcb = os_tcb
    os_mem_data = os_mem_data
    Proximity2Header_s = Proximity2Header_s
    StreamingBroadcastReq_t__structure = StreamingBroadcastReq_t__structure
    os_mbox_data = os_mbox_data
    os_stk_data = os_stk_data
    ...
    OS_SEM_DATA = OS_SEM_DATA
    OS_FLAG_GRP = OS_FLAG_GRP
    Proximity2Header_t = Proximity2Header_t
    OS_MEM_DATA = OS_MEM_DATA
    StreamingBroadcastReq_t = StreamingBroadcastReq_t
    OS_TMR_WHEEL = OS_TMR_WHEEL
    OS_STK_DATA = OS_STK_DATA
    OS_TMR = OS_TMR
    ...
    def FilterOutUnwantedSp(self, payload):
        '''
        Arguments:
        -payload - PointerType("StreamingBroadcastReq_t")
        Return type:
        -bool
        Declaration line: 57
        '''
    ...
    def StreamValidateBcBeacon(self, payload):
        '''
        Arguments:
        -payload - PointerType('StreamingBroadcastReq_t')
        Return type:
    ...
    def StreamPairProc(self, irqStatus):
        '''
        Arguments:
        -irqStatus - u32
        Return type:
    ...
    def StreamValidatePairingResp(self, response):
        '''
        Arguments:
        -response - PointerType("u8")
        Return type:
        -bool
        Declaration line: 91
        '''
    ...
    def StreamPairInit(self, ctx):
        '''
        Arguments:
        -ctx - PointerType('StreamingPairingCtx_t')
        Return type:
        -None
    ...
    def StreamingCreatePairingRequest(self, req, beacon):
        '''
        Arguments:
        -req - PointerType("StreamingPairReq_t")
        -beacon - PointerType('StreamingBroadcastReq_t')
        Return type:
        -None
    ...
    pairingCtx = StreamingPairingCtx_t
    bcAddr = u8 * 4
    pairingPfc = u8 * 4
    broadcastBeacon = StreamingBroadcastReq_t
    rtPairingCtx = StreamingRtPairingCtx_t
    ...
    def __init__(self, device):
        ...
        ### Variables ###
        #################
        self.pairingCtx = StaticVariable(device, self.StreamingPairingCtx_t, 0x7fe0372c, False)
        self.bcAddr = StaticVariable(device, u8 * 4, 0x7fe0369c, False)
        self.pairingPfc = StaticVariable(device, u8 * 4, 0x7fe036a0, False)
        self.broadcastBeacon = StaticVariable(device, self.StreamingBroadcastReq_t, 0x7fe0377c, False)
        self.rtPairingCtx = StaticVariable(device, self.StreamingRtPairingCtx_t, 0x7fe03764, False)

        ######################
        ### Functions data ###
        ######################
        self.FilterOutUnwantedSp = StaticFunction(device, 0x8100a87cL, thumb=0, name='FilterOutUnwantedSp', project='streaming_functions', return_type=bool, size=88, line=57, arg_list=[('payload',PointerType("StreamingBroadcastReq_t"))])
        self.StreamValidateBcBeacon = StaticFunction(device, 0x8100a8d4L, thumb=0, name='StreamValidateBcBeacon', project='streaming_functions', return_type=bool, size=76, line=78, arg_list=[('payload',PointerType('StreamingBroadcastReq_t'))])
        self.StreamPairProc = StaticFunction(device, 0x8100ab50L, thumb=0, name='StreamPairProc', project='streaming_functions', return_type=None, size=1012, line=172, arg_list=[('irqStatus',u32)])
        self.StreamValidatePairingResp = StaticFunction(device, 0x8100a920L, thumb=0, name='StreamValidatePairingResp', project='streaming_functions', return_type=bool, size=136, line=91, arg_list=[('response',PointerType("u8"))])
        ...
        self.StreamPairInit = StaticFunction(device, 0x8100aad4L, thumb=0, name='StreamPairInit', project='streaming_functions', return_type=None, size=124, line=156, arg_list=[('ctx',PointerType('StreamingPairingCtx_t'))])
        self.StreamingCreatePairingRequest = StaticFunction(device, 0x8100a9a8L, thumb=0, name='StreamingCreatePairingRequest', project='streaming_functions', return_type=None, size=300, line=121, arg_list=[('req',PointerType("StreamingPairReq_t")),('beacon',PointerType('StreamingBroadcastReq_t'))])
