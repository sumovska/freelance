...
var controller, ASPECT_RATIO, IMAGE_HEIGHT;

$(document).ready(function ($) {
	$('.js-popup').click(function () {
		var where = $(this).attr("href").replace(/^.*#(.*)/, "$1"), target = $('.popup-' + where);
		$('.popup').hide();
		target.show();
		return false;
	});

	$('.bt-menu-trigger').addClass('bt-menu-trigger-0');
	...
	$('.wrapper').fullpage({
		...
		onLeave: function (index, newIndex, direction) {
			$('.bt-menu-trigger').attr('class', 'bt-menu-trigger');
			$('#fullPage-nav').attr('class', 'right');
			if (direction == 'down') {
				$('.section').eq(index - 1).addClass('active-down');
				$('.bt-menu-trigger').addClass('bt-menu-trigger-' + (index));
				$('#fullPage-nav').addClass('fullPage-nav-' + (index));
			} else if (direction == 'up') {
				$('.bt-menu-trigger').addClass('bt-menu-trigger-' + (index - 2));
				$('#fullPage-nav').addClass('fullPage-nav-' + (index - 2));
			}
			$('.section').eq(index - 2).removeClass('active-down');
		}
	});
	...
	$("#audit-form").each(function () {
		window.result = '';
		var _self = $(this);
		$('button.progress-button', this).each(function () {
